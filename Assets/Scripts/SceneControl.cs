...
public static class SceneControl
{
    ...
    private static DEPosition beginPos = new DEPosition(); //the position where selection begins
    private static DEDirection beginDir;
    private static DEPosition endPos = new DEPosition();
    ...
    public static void StartSelecting()
    {
        ...
        if (BaseControl.GetHitInfo(mouseRay, out startPos, out startDir)) {
            //Debug.Log("hit");
            if (beginPos.Equals(startPos) && beginDir.Equals(startDir) && actualSelect.Count == 1) {
    ...
    {
        if (endPos == null) return true;
        mouseRay = mainCam.ScreenPointToRay(Input.mousePosition);
        DEPosition stopPos;
        DEDirection stopDir;
        if (BaseControl.GetHitInfo(mouseRay, out stopPos, out stopDir)) {
            if (endPos.Equals(stopPos) && endDir.Equals(stopDir)) {
                //UnityEngine.Debug.Log("rendering skipped");
                endPos = stopPos;
                endDir = stopDir;
                return false;
            } else {
    ...
    public static void UpdateSelecting()
    {
        ...
        //UnityEngine.Debug.Log("rendering can not be skipped");
        PlaneCheck();
        SetSelection();
    }
    ...
    private static void PlaneCheck()
    {
        ...
        if (beginDir == endDir) {
            ...
            if (beginDir == DEDirection.front || beginDir == DEDirection.back) {
    ...
    private static void SetSelection()
    {
        ...
        selection = DEUtility.GetSelectGroup(beginPos, beginDir, endPos, endDir);
        actualSelect.Clear();
        //to be optimized
