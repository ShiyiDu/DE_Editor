...
public static class DEUtility
{
	...
	public static DEPosition[] GetSelectGroup(DEPosition beginPos, DEDirection beginDir, DEPosition endPos, DEDirection endDir)
	{
		...
		foreach (string str in closest) {
			switch (str) {
				...
				case "left":
					...
					break;
				case "forward":
					differences[2] = Differences(mousePosition, DEPosition.GetQuadWorldVertexs(position, DEDirection.forward, l2W));
					break;
				case "back":
