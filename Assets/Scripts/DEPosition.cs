...
public struct DEPosition
{
    ...
    {
        return (_x) + (_y << 10) + (_z << 20); //this one is 3 times faster with small amount of data, what about large set of data?
        //return (_x.GetHashCode() + 2) * (_y.GetHashCode() + 3) * (_z.GetHashCode() + 5);
        //which one of above is a better hashing function?

        //by rewriting the hashing function, the handle drawer runs more than 20x faster!!
